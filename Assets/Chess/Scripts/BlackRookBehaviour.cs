...
public class BlackRookBehaviour : MonoBehaviour {
				...
				for (int i = 0; i < 8; i++) {
					for (int j = 0; j < 8; j++) {
						if (table.table [i].rows [j].GetComponent<SquareBehaviour> ().moveHere) {
							if (king.GetComponent<BlackKingBehaviour> ().leftCastling) {
								king.GetComponent<BlackKingBehaviour> ().leftCastling = !left && right;
							}
							if (king.GetComponent<BlackKingBehaviour> ().rightCastling) {
								king.GetComponent<BlackKingBehaviour> ().rightCastling = left && !right;
							}
