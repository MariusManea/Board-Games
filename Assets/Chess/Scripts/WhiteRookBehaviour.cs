...
public class WhiteRookBehaviour : MonoBehaviour {

	public GameObject position, transformWindow, king;
	public bool selected, left, right;
	public int X, Y;
				...
				for (int i = 0; i < 8; i++) {
					for (int j = 0; j < 8; j++) {
						if (table.table [i].rows [j].GetComponent<SquareBehaviour> ().moveHere) {
							if (king.GetComponent<WhiteKingBehaviour> ().leftCastling) {
								king.GetComponent<WhiteKingBehaviour> ().leftCastling = !left && right;
							}
							if (king.GetComponent<WhiteKingBehaviour> ().rightCastling) {
								king.GetComponent<WhiteKingBehaviour> ().rightCastling = left && !right;
							}
